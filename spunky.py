...
Python 2 → Python 3 port : PtitBigorneau https://ptitbigorneau.fr
...
Modify the files '/conf/config.ini' and '/conf/rules.txt'
...
#############################################################################################################################
def check_iamgod():
    curs.execute("SELECT COUNT(*) FROM `urtstats` WHERE `admin_role` = 100")
...
try:
    ...
    settings = "config.ini"
config_file = os.path.join(home, 'conf', settings)
config = configparser.ConfigParser()
config.read(config_file)
# load the GEO database and store it globally in interpreter memory
geoip2_path = config.get('geoip2', 'path') if config.has_option('geoip2', 'path') else "lib/GeoLite2-City.mmdb"
geoip = geoip2.database.Reader(geoip2_path)
# commands
try:
    for key in config['commands']:
        values = config["commands"][key].split(", ")
        cmd_data = {}

        try:
            cmd_data['desc'] = values[0]
        except:
            pass
        try:
            cmd_data['syntax'] = values[1]
        except:
            pass
        try:
            cmd_data['level'] = int(values[2])
        except:
            pass        
        try:
            cmd_data['short'] = values[3]
        except:
            pass
        
        commands[key] = cmd_data
    ...
    commands = {'help': {'desc': 'display all available commands', 'syntax': '^7Usage: ^2!help', 'level': 0, 'short': 'h'},
                ...
                'urtstats': {'desc': 'display full player statistics', 'syntax': '^7Usage: ^2!urtstats ^7[<name>]', 'level': 1},
                'urttopstats': {'desc': 'display the top players', 'syntax': '^7Usage: ^2!urttopstats', 'level': 1, 'short': 'topstats'},
                ...
                'clear': {'desc': 'clear all player warnings', 'syntax': '^7Usage: ^2!clear', 'level': 80, 'short': 'clear'},
...
# reasons
try:
    for key in config['reasons']:
        reasons[key] = config["reasons"][key]
except:
...
## player commands
            ...
            # urtstats - display full player stats
            elif sar['command'] == '!urtstats':
                if line.split(sar['command'])[1]:
            ...
            # urttopstats
            elif (sar['command'] in ('!urttopstats', '!topstats')) and self.game.players[sar['player_num']].get_admin_role() >= commands['urttopstats']['level']:
                values = (time.strftime("%Y-%m-%d %H:%M:%S", time.localtime((time.time() - 10368000))),)  # last played within the last 120 days
                result = curs.execute("SELECT name FROM `urtstats` WHERE (`rounds` > 35 or `kills` > 500) and `last_played` > ? ORDER BY `ratio` DESC LIMIT 3", values).fetchall()
                toplist = ['^1#%s ^7%s' % (index + 1, result[index][0]) for index in range(len(result))]
...
## mod level 20
            ...
            # list - list all connected players
                ...
                msg = "^7Players online: %s" % ", ".join(["^3%s [^2%d^3]" % (player.get_name(), player.get_player_num()) for player in self.game.players.values() if player.get_player_num() != bot_player_num])
                self.game.rcon_tell(sar['player_num'], msg)
...
## admin level 40
            ...
            # !regulars/!regs - display the regular players online
                ...
                liste = "%s" % ", ".join(["^3%s [^2%d^3]" % (player.get_name(), player.get_admin_role()) for player in self.game.players.values() if player.get_admin_role() == 2])
...
## senior admin level 80
            ...
            # !addbots
                ...
                self.game.send_rcon('bot_minplayers %s'%self.botminplayers)
...
#########################################################################################################################################################"
    ...
    def get_admins_online(self):
        ...
        """
        liste = "%s" % ", ".join(["^3%s [^2%d^3]" % (player.get_name(), player.get_admin_role()) for player in self.game.players.values() if player.get_admin_role() >= 20])
...
class Player(object):
    ...
    def __init__(self, player_num, ip_address, guid, name, auth=''):
        ...
        # GeoIP lookup
        try:
            infogeoip = geoip.city(ip_address)
            self.country = "%s (%s)" % (infogeoip.country.name, infogeoip.country.iso_code)
        except:
            self.country = None
    ...
    def save_info(self):
        if self.registered_user:
            ...
            curs.execute("UPDATE `urtstats` SET `kills` = ?,`deaths` = ?,`headshots` = ?,`team_kills` = ?,`team_death` = ?,`max_kill_streak` = ?,`suicides` = ?,`rounds` = `rounds` + 1,`ratio` = ? WHERE `guid` = ?", values)
            conn.commit()
    ...
    def check_database(self):
        ...
        # check urtstats table
        values = (self.guid,)
        curs.execute("SELECT COUNT(*) FROM `urtstats` WHERE `guid` = ?", values)
        if int(curs.fetchone()[0]) == 0:
            self.registered_user = False
        else:
            self.registered_user = True
            # get DB DATA for urtstats
            values = (self.guid,)
            curs.execute("SELECT `last_played`,`num_played`,`kills`,`deaths`,`headshots`,`team_kills`,`team_death`,`max_kill_streak`,`suicides`,`admin_role`,`first_seen` FROM `urtstats` WHERE `guid` = ?", values)
            result = curs.fetchone()
            ...
            curs.execute("UPDATE `urtstats` SET `name` = ?,`last_played` = ?,`num_played` = `num_played` + 1 WHERE `guid` = ?", values)
            conn.commit()
    ...
    def define_offline_player(self, player_id):
        ...
        if int(curs.fetchone()[0]) == 0:
            ...
            curs.execute("SELECT `last_played`,`admin_role` FROM `urtstats` WHERE `guid` = ?", values)
            result = curs.fetchone()
            self.last_visit = result[0]
    ...
    def register_user_db(self, role=1):
        if not self.registered_user:
            ...
            curs.execute("INSERT INTO `urtstats` (`guid`,`name`,`ip_address`,`first_seen`,`last_played`,`num_played`,`admin_role`) VALUES (?,?,?,?,?,1,?)", values)
            conn.commit()
            self.registered_user = True
    ...
    def update_db_admin_role(self, role):
        ...
        curs.execute("UPDATE `urtstats` SET `admin_role` = ? WHERE `guid` = ?", values)
        conn.commit()
...
class Game(object):
    ...
    def __init__(self, config_file, urt_modversion):
        ...
        if self.dynamic_mapcycle:
            ...
            self.big_cycle = list(filter(None, game_cfg.get('mapcycle', 'big_cycle').replace(' ', '').split(','))) if game_cfg.has_option('mapcycle', 'big_cycle') else []
            self.small_cycle = list(filter(None, game_cfg.get('mapcycle', 'small_cycle').replace(' ', '').split(','))) if game_cfg.has_option('mapcycle', 'small_cycle') else []
